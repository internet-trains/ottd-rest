...
from libottdadmin2.packets.admin import AdminChat, AdminPoll, AdminUpdateFrequency, PollExtra, AdminGamescript
...
async def server_poll(timeout):
    while True:
        ...
        poll_packet = AdminPoll.create(type=UpdateType.COMPANY_ECONOMY, extra=PollExtra.ALL)
...
async def b4_1st_req():
    ...
    app.ottd_client.send_packet(AdminUpdateFrequency.create(type=UpdateType.COMPANY_ECONOMY, freq=UpdateFrequency.POLL))
    app.ottd_client.send_packet(AdminUpdateFrequency.create(type=UpdateType.GAMESCRIPT, freq=UpdateFrequency.AUTOMATIC))
...
async def change_company_name(company_id):
    packet = AdminGamescript.create(json_data={
	"action": "call",
